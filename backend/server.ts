...
// ── Autoloop / Ralph session monitoring ──────────────────────────────────────
    ...
    for (const childPid of children) {
      try {
        ...
        if (childCwd && childCwd !== paneCwd) {
          // Check if this dir has .autoloop/
...
function scanLoopSessions(): LoopSession[] {
  ...
  // Group panes by tmux session name — only process each session once
  ...
  for (const [sname, paneList] of sessionPanes) {
    // Capture output from the first pane (main orchestrator pane)
    ...
    // Collect all unique project dirs from all panes in this session
    ...
    // Collect all active runs from all registries found across project dirs
    const seenRunIds = new Set<string>()
    let foundAnyRun = false

    // Get tmux session creation time as fallback startedAt
    ...
    for (const projectDir of projectDirs) {
      ...
      // Read registry and find all active runs
      let regEntries: any[] = []
      try {
        const regRaw = readFileSync(registryPath, 'utf-8')
        const regLines = regRaw.trim().split('\n').filter(Boolean)
        // Parse all entries, keep the latest state per run_id
      ...
      // Process each active run from this registry
      for (const entry of regEntries) {
        if (entry.status !== 'running') continue
        if (seenRunIds.has(entry.run_id)) continue

        // Liveness check: verify the run's pid is still alive
        let isAlive = false
        if (entry.pid) {
          try {
            process.kill(entry.pid, 0) // signal 0 = check existence
            isAlive = true
          } catch {
            isAlive = false
          }
...
// Helper: extract tmux session name from loop ID (format: "tmuxSession/runId" or just "tmuxSession")
...
app.get('/api/loops/:id/journal', (req: Request, res: Response) => {
  ...
  try {
    ...
    // Find journal file: match specific runId if provided
...
function _wireSlotEvents(pi: PiProcess, slotKey: string): void {
  ...
  pi.on('message_end', (event: any) => {
    if (event.message?.role === 'custom') {
      ...
      // If a subagent/process result arrives outside of a turn, the extension's
      // triggerTurn likely failed or wasn't set. Send a follow-up prompt to
      // ensure the agent gets a turn to react to the result.
      const TURN_TRIGGER_TYPES = ['subagent-result', 'ad-process:update']
      if (!midTurn && TURN_TRIGGER_TYPES.includes(m.customType)) {
        // Small delay to avoid racing with an internally-triggered turn
        // that might be about to emit agent_start
        setTimeout(() => {
          if (!midTurn && pi.proc && !pi.proc.killed && pi.proc.exitCode === null) {
            // Send a prompt to trigger a new agent turn. The custom message
            // content is already in the conversation context from message_end.
