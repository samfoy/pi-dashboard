...
export default function WelcomeView({
  ...
  return (
    <div className="flex-1 flex flex-col items-center justify-center gap-5 px-8">
      ...
      {/* Pickers row */}
      <div className="flex gap-4 flex-wrap justify-center w-full max-w-[700px]">
        {/* Model picker */}
      ...
      <div className="w-full max-w-[700px]">
        ...
        <div className="flex gap-2.5 items-end">
          ...
          <textarea ref={inputRef} aria-label="Message input" className={`flex-1 bg-bg-elevated border border-border rounded-lg px-4 py-3 text-text text-sm font-body outline-none min-h-[44px] leading-normal transition-all focus-ring placeholder:text-muted ${prefillHint ? 'resize-y max-h-[50vh]' : 'resize-none max-h-[140px]'}`} placeholder="Message Pi…" rows={1} value={input}
          ...
          <button className="btn-sweep bg-accent text-white border-none rounded-lg px-5 h-[44px] text-sm font-semibold cursor-pointer hover:bg-accent-hover hover:shadow-[0_0_20px_var(--accent-glow)] disabled:opacity-30 disabled:cursor-not-allowed transition-all font-body" onClick={() => send()} disabled={!input.trim()}>Send</button>
        </div>
      </div>
    </div>
  )
