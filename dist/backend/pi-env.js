...
function sqliteQuery(sql) {
    if (!existsSync(MEMORY_DB))
        return [];
    try {
        const raw = execSync(`sqlite3 -json "${MEMORY_DB}" ${JSON.stringify(sql)}`, { encoding: 'utf-8', timeout: 5000 });
...
export function getLessons(limit = 100) {
    return sqliteQuery(`SELECT id, rule, category, negative, created_at FROM lessons WHERE is_deleted = 0 ORDER BY created_at DESC LIMIT ${limit}`);
}
export function getFacts() {
    return sqliteQuery('SELECT key, value, confidence, source, updated_at FROM semantic ORDER BY updated_at DESC');
}
...
export function getCrontab() {
    try {
        ...
        return raw.split('\n')
            ...
            if (!parts)
                return { schedule: '', command: line, raw: line };
...
export function getVaultStats() {
    ...
    try {
        ...
        if (existsSync(tn))
            stats.taskNotes = readdirSync(tn).filter(f => f.endsWith('.md')).length;
        const mn = join(VAULT_DIR, getVaultSubdir('meetings'));
        if (existsSync(mn))
            stats.meetingNotes = readdirSync(mn).filter(f => f.endsWith('.md')).length;
        const pn = join(VAULT_DIR, getVaultSubdir('people'));
        if (existsSync(pn))
            stats.persons = readdirSync(pn).filter(f => f.endsWith('.md')).length;
        const rn = join(VAULT_DIR, getVaultSubdir('recipes'));
        if (existsSync(rn))
            stats.recipes = readdirSync(rn).filter(f => f.endsWith('.md')).length;
    }
    catch { /* skip on error */ }
    return stats;
}
