...
import { readdirSync, statSync, readFileSync, writeFileSync, mkdirSync, unlinkSync, watch as fsWatch } from 'fs';
import { readFile, writeFile, mkdir } from 'fs/promises';
import os from 'os';
import { execSync } from 'child_process';
import { PiManager } from './pi-manager.js';
import { shutdownAll as shutdownPty } from './pty-manager.js';
...
function searchSessionIndex(query, limit) {
    ...
    try {
        ...
        try {
            const rows = db.prepare('SELECT id FROM s WHERE s MATCH ? ORDER BY bm25(s) LIMIT ?').all(ftsQuery, limit);
...
function addNotification(notif) {
    ...
    if (notifications.length > NOTIF_MAX)
        notifications.splice(0, notifications.length - NOTIF_MAX);
...
if (savedSlots.length > 0)
    console.log(`   Restored ${savedSlots.length} chat slot(s)`);
...
function createVersion(filePath, content) {
    ...
    if (!versions) {
        versions = [];
        versionStore.set(filePath, versions);
    }
...
app.get('/api/chat/slots/:key/tree', (req, res) => {
    ...
    if (!sessionPath)
        return res.json({ entries: [], leafId: null });
...
app.patch('/api/chat/slots/:key/tags', (req, res) => {
    const pi = manager.getSlot(req.params.key);
    if (!pi)
        return res.status(404).json({ error: 'slot not found' });
    ...
    broadcastSlots();
    broadcast('slot_tags', { key: req.params.key, tags: pi._tags });
    persistSlots();
...
app.put('/api/pi/file', (req, res) => {
    const { path: filePath, content } = req.body;
    if (!filePath || content == null || filePath.includes('..'))
...
app.get('/api/chat/slots/:key/system-prompt', async (req, res) => {
    const pi = manager.getSlot(req.params.key);
    if (!pi)
        return res.status(404).json({ error: 'slot not found' });
    try {
        ...
        try {
            ...
            const piMemoryPkg = piMemoryCandidates.find(p => { try {
                statSync(join(p, 'package.json'));
                return true;
            }
            catch {
                return false;
            } }) || piMemoryCandidates[0];
            ...
            try {
                ...
                if (localPath)
                    dbPath = join(localPath, 'memory.db');
            }
            catch { }
...
app.get('/api/subagents/:id/log', (req, res) => {
    try {
    ...
    try {
        ...
        for (const pane of panes) {
            ...
            try {
                ...
                if (files.length > 0)
                    sessionFile = join(sessPath, files[0]);
            }
            catch { }
            sessions.push({
                tmuxSession: sname,
...
app.post('/api/save-image', async (req, res) => {
    ...
    if (!data || !rawPath)
        return res.status(400).json({ error: 'data and path required' });
...
app.get('/api/file-version', (req, res) => {
    ...
    if (!filePath || isNaN(ver))
        return res.status(400).json({ error: 'path and version required' });
...
app.post('/api/file-comments', async (req, res) => {
    ...
    if (!filePath || !Array.isArray(comments))
        return res.status(400).json({ error: 'path and comments array required' });
    const dir = dirname(filePath);
    const sidecar = join(dir, '.' + basename(filePath) + '.comments.json');
    try {
...
app.get('/api/local-file', (req, res) => {
    ...
    res.sendFile(resolved, { root: '/' }, (err) => { if (err && !res.headersSent)
...
app.get('/api/styles', (_req, res) => {
    try {
        ...
        let active = '';
        try {
            active = readFileSync(ACTIVE_STYLE_FILE, 'utf-8').trim();
        }
        catch { }
        res.json({ styles: files, active });
    }
    catch {
        res.json({ styles: [], active: '' });
    }
});
app.get('/api/styles/:name', (req, res) => {
...
app.put('/api/styles/:name', express.json(), async (req, res) => {
    const name = req.params.name;
    if (!name || /[/\\]/.test(name) || name.startsWith('.'))
        return res.status(400).json({ error: 'invalid name' });
    const css = req.body?.css;
    if (typeof css !== 'string')
        return res.status(400).json({ error: 'css required' });
...
app.delete('/api/styles/:name', (req, res) => {
    ...
    if (!name || /[/\\]/.test(name))
        return res.status(400).json({ error: 'invalid name' });
    try {
        unlinkSync(join(STYLES_DIR, name + '.css'));
    }
    catch { }
    // Clear active if it was this style
    try {
        if (readFileSync(ACTIVE_STYLE_FILE, 'utf-8').trim() === name)
            writeFileSync(ACTIVE_STYLE_FILE, '', 'utf-8');
    }
    catch { }
    res.json({ ok: true });
});
app.put('/api/styles-active', express.json(), async (req, res) => {
    const name = req.body?.name ?? '';
    await writeFile(ACTIVE_STYLE_FILE, name, 'utf-8');
    res.json({ ok: true });
});
// ─── Knowledge Graph ─────────────────────────────────────────────────
const GRAPH_DB_PATH = join(os.homedir(), 'scratch', 'graph-memory', 'data', 'graph.db');
let _graphDb = null;
function getGraphDb() {
    if (_graphDb)
        return _graphDb;
    try {
        statSync(GRAPH_DB_PATH);
        _graphDb = new DatabaseSync(GRAPH_DB_PATH, { open: true });
        return _graphDb;
    }
    catch {
        return null;
...
app.get('/api/graph/stats', (_req, res) => {
    const db = getGraphDb();
    if (!db)
        return res.json({ entities: 0, edges: 0, byType: {}, edgesByType: {} });
    const entities = db.prepare('SELECT COUNT(*) as c FROM entities').get().c;
    const edges = db.prepare('SELECT COUNT(*) as c FROM edges').get().c;
    const byType = {};
    for (const r of db.prepare('SELECT type, COUNT(*) as c FROM entities GROUP BY type ORDER BY c DESC').all()) {
        byType[r.type] = r.c;
    }
    const edgesByType = {};
    for (const r of db.prepare('SELECT type, COUNT(*) as c FROM edges GROUP BY type ORDER BY c DESC').all()) {
        edgesByType[r.type] = r.c;
    }
    res.json({ entities, edges, byType, edgesByType });
});
app.get('/api/graph/search', (req, res) => {
    ...
    let rows = [];
    const ftsQuery = q.replace(/[^\w\s]/g, ' ').split(/\s+/).filter(Boolean).map(t => `"${t}"*`).join(' OR ');
    if (ftsQuery) {
        try {
            const sql = type
                ? `SELECT e.id, e.type, e.name, e.properties,
             (SELECT COUNT(*) FROM edges WHERE source_id = e.id OR target_id = e.id) as edgeCount
           FROM entity_fts f JOIN entities e ON f.rowid = e.rowid
           WHERE f.entity_fts MATCH ? AND e.type = ? ORDER BY rank LIMIT ?`
                : `SELECT e.id, e.type, e.name, e.properties,
             (SELECT COUNT(*) FROM edges WHERE source_id = e.id OR target_id = e.id) as edgeCount
           FROM entity_fts f JOIN entities e ON f.rowid = e.rowid
           WHERE f.entity_fts MATCH ? ORDER BY rank LIMIT ?`;
            rows = type
                ? db.prepare(sql).all(ftsQuery, type, limit)
                : db.prepare(sql).all(ftsQuery, limit);
        }
        catch { /* FTS failed, fallback below */ }
    }
    if (rows.length === 0) {
        const like = `%${q}%`;
        const sql = type
            ? `SELECT id, type, name, properties,
           (SELECT COUNT(*) FROM edges WHERE source_id = entities.id OR target_id = entities.id) as edgeCount
         FROM entities WHERE name LIKE ? AND type = ? LIMIT ?`
            : `SELECT id, type, name, properties,
           (SELECT COUNT(*) FROM edges WHERE source_id = entities.id OR target_id = entities.id) as edgeCount
         FROM entities WHERE name LIKE ? LIMIT ?`;
        rows = type
            ? db.prepare(sql).all(like, type, limit)
            : db.prepare(sql).all(like, limit);
    }
    res.json(rows.map(r => ({ id: r.id, type: r.type, name: r.name, properties: r.properties, edgeCount: r.edgeCount })));
});
app.get('/api/graph/entity/:id', (req, res) => {
    const db = getGraphDb();
    if (!db)
        return res.status(404).json({ error: 'graph db not found' });
    const id = req.params.id;
    const entity = db.prepare('SELECT * FROM entities WHERE id = ?').get(id);
    if (!entity)
        return res.status(404).json({ error: 'entity not found' });
    const edges = db.prepare(`SELECT ed.*,
       s.name as sourceName, s.type as sourceType,
       t.name as targetName, t.type as targetType
     FROM edges ed
     LEFT JOIN entities s ON s.id = ed.source_id
     LEFT JOIN entities t ON t.id = ed.target_id
     WHERE ed.source_id = ? OR ed.target_id = ?`).all(id, id);
    res.json({ entity, edges });
});
app.get('/api/graph/neighbors', (req, res) => {
    ...
    const depth = Math.min(parseInt(String(req.query.depth || '1'), 10) || 1, 4);
    const limit = Math.min(parseInt(String(req.query.limit || '50'), 10) || 50, 200);
    // Recursive CTE to find N-hop neighbors
    const nodeRows = db.prepare(`
    WITH RECURSIVE hops(eid, d) AS (
      VALUES(?, 0)
      UNION
      SELECT CASE WHEN e.source_id = h.eid THEN e.target_id ELSE e.source_id END, h.d + 1
      FROM edges e JOIN hops h ON e.source_id = h.eid OR e.target_id = h.eid
      WHERE h.d < ?
    )
    SELECT DISTINCT ent.id, ent.type, ent.name,
      (SELECT COUNT(*) FROM edges WHERE source_id = ent.id OR target_id = ent.id) as edgeCount
    FROM hops JOIN entities ent ON ent.id = hops.eid
    LIMIT ?
  `).all(id, depth, limit);
    ...
    const edgeRows = nodeIds.size > 0
        ? db.prepare(`SELECT DISTINCT id, source_id as source, target_id as target, type, weight
         FROM edges WHERE source_id IN (${placeholders}) AND target_id IN (${placeholders})`).all(...nodeIds, ...nodeIds)
        : [];
    res.json({
        nodes: nodeRows.map(n => ({ id: n.id, type: n.type, name: n.name, edgeCount: n.edgeCount })),
        edges: edgeRows.map(e => ({ source: e.source, target: e.target, type: e.type, weight: e.weight })),
    });
});
app.get('/api/graph/path', (req, res) => {
    ...
    const maxDepth = Math.min(parseInt(String(req.query.maxDepth || '4'), 10) || 4, 6);
    // BFS shortest path via recursive CTE
    const pathRows = db.prepare(`
    WITH RECURSIVE bfs(eid, path_ids, path_edges, d) AS (
      VALUES(?, json_array(?), json_array(), 0)
      UNION ALL
      SELECT
        CASE WHEN e.source_id = b.eid THEN e.target_id ELSE e.source_id END,
        json_insert(b.path_ids, '$[#]', CASE WHEN e.source_id = b.eid THEN e.target_id ELSE e.source_id END),
        json_insert(b.path_edges, '$[#]', json_object('source', e.source_id, 'target', e.target_id, 'type', e.type)),
        b.d + 1
      FROM edges e JOIN bfs b ON (e.source_id = b.eid OR e.target_id = b.eid)
      WHERE b.d < ?
        AND json_array_length(b.path_ids) = b.d + 1
        AND NOT EXISTS (
          SELECT 1 FROM json_each(b.path_ids) j
          WHERE j.value = CASE WHEN e.source_id = b.eid THEN e.target_id ELSE e.source_id END
        )
    )
    SELECT path_ids, path_edges FROM bfs WHERE eid = ? LIMIT 5
  `).all(from, from, maxDepth, to);
    const paths = pathRows.map(row => {
        const nodeIds = JSON.parse(row.path_ids);
        const edges = JSON.parse(row.path_edges);
        // Resolve node names
        const nodes = nodeIds.map(nid => {
            const ent = db.prepare('SELECT id, type, name FROM entities WHERE id = ?').get(nid);
            return ent || { id: nid, type: 'unknown', name: nid };
        });
        const desc = nodes.map((n, i) => {
            if (i === 0)
                return n.name;
            const edge = edges[i - 1];
            return edge ? ` --${edge.type}--> ${n.name}` : ` --> ${n.name}`;
        }).join('');
...
app.get('/api/graph/top', (req, res) => {
    ...
    const sql = type
        ? `SELECT e.id, e.type, e.name,
         (SELECT COUNT(*) FROM edges WHERE source_id = e.id OR target_id = e.id) as edgeCount
       FROM entities e WHERE e.type = ? ORDER BY edgeCount DESC LIMIT ?`
        : `SELECT e.id, e.type, e.name,
         (SELECT COUNT(*) FROM edges WHERE source_id = e.id OR target_id = e.id) as edgeCount
       FROM entities e ORDER BY edgeCount DESC LIMIT ?`;
    const rows = type
        ? db.prepare(sql).all(type, limit)
        : db.prepare(sql).all(limit);
    res.json(rows.map(r => ({ id: r.id, type: r.type, name: r.name, edgeCount: r.edgeCount })));
});
// ── Static files ──
...
wss.on('connection', (ws) => {
    ...
    ws.on('message', (raw) => {
        try {
            ...
            if (msg.type === 'watch_file' && msg.path)
                startWatching(msg.path, ws);
            if (msg.type === 'unwatch_file' && msg.path)
                stopWatching(msg.path, ws);
        }
        catch { /* ignore */ }
    });
    ws.on('close', () => {
...
function _wireSlotEvents(pi, slotKey) {
    ...
    const _throttledPersist = () => { if (!_persistTimer)
        _persistTimer = setTimeout(() => { _persistTimer = null; persistSlots(); }, 5000); };
    ...
    pi.on('message_update', ({ event, delta: _d }) => {
        if (event?.assistantMessageEvent?.type === 'thinking_end' && thinkingBuf) {
            ...
            if (_partialThinkMsg)
                _partialThinkMsg._partial = false;
    ...
    pi.on('tool_start', ({ toolName, toolCallId, args }) => {
        ...
        if (_partialTextMsg) {
            _partialTextMsg._partial = false;
            _partialTextMsg = null;
        }
    ...
    pi.on('extension_ui', (event) => {
        if (event.method === 'confirm') {
            ...
            if (first)
                pi.send({ type: 'extension_ui_response', id: event.id, value: first });
        }
    ...
    pi.on('exit', (code) => {
        ...
        if (midTurn) {
            ...
            _partialTextMsg = null;
            _partialThinkMsg = null;
            const stderr = pi._stderrLines?.slice(-5)?.join('\n') || '';
...
process.on('SIGINT', () => {
    saveSlotStateSync(manager.slots);
