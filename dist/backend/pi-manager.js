    ...
    start() {
        ...
        this.proc.on('exit', (code, signal) => {
            ...
            if (this._stoppingTimer) {
                clearTimeout(this._stoppingTimer);
                this._stoppingTimer = null;
            }
                ...
                if (event.messages) {
                    for (const m of event.messages) {
                        if (m.role === 'assistant') {
                            ...
                            const toolMsg = [...this.messages].reverse().find((msg) => msg.role === 'tool' && msg.meta?.toolCallId === m.toolCallId);
                            if (toolMsg) {
                                let resultText = '';
                                if (Array.isArray(m.content)) {
...
export class PiManager {
    ...
    listSlots() {
        return Array.from(this.slots.entries()).map(([key, pi]) => {
            ...
            const updatedAt = lastMsg?.ts || pi._lastActivity ? new Date(Math.max(lastMsg?.ts ? new Date(lastMsg.ts).getTime() : 0, pi._lastActivity || 0)).toISOString() : createdAt;
            return {
                key,
